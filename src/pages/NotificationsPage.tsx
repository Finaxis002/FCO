...
import { Bell, CheckCircle2, FilePlus, UserPlus, AlertCircle, Activity, Eye, Trash2 } from "lucide-react";
...
const NOTIFICATION_ICONS_PAGE: Record<AppNotification['type'], React.ElementType> = {
  update: CheckCircle2,
  creation: FilePlus,
  assign: UserPlus,
...
useEffect(() => {
 ...
 const markAsRead = async (id: string) => {
  ...
  if (loading) {
    return (
      <>
        <PageHeader title="Notifications" description="Loading your recent activity..." />
        <Card>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction onClick={deleteAllNotifications} className="bg-destructive hover:bg-destructive/90 text-destructive-foreground">Delete All</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
           )}
            ...
            <ScrollArea className="h-[calc(100vh-280px)] pr-4"> {/* Adjust height as needed */}
              <ul className="space-y-4">
                {notifications.map((notification) => {
                  ...
                  const messagePrefix = caseName ? <RouterLink to={`/cases/${notification.caseId}`} className="font-semibold text-primary hover:underline">{`Case '${caseName}'`}</RouterLink> : "";
                  ...
                  return (
                    ...
                    >
                      <Avatar className="h-10 w-10 mt-1">
                        <AvatarFallback className={`${!notification.read ? 'bg-primary/20 text-primary' : 'bg-muted text-muted-foreground'}`}>
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                {messagePrefix}{messagePrefix && ": "}{notification.message}
                                </p>
                            ...
                            {!notification.read && (
                                <Badge variant="default" className="text-xs h-5 shrink-0">New</Badge>
                            )}
                        </div>
                        ...
                        <div className="mt-3 flex gap-2 flex-wrap">
                          ...
                          {!notification.read && (
                           ...
                           <AlertDialog>
                              <AlertDialogTrigger asChild>
                                 <Button variant="ghost" size="xs" className="text-destructive hover:text-destructive hover:bg-destructive/10 px-2 py-1">
