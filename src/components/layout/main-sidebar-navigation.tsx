...
export default function MainSidebarNavigation({ isMobile = false }: MainSidebarNavigationProps) {
  ...
  const renderNavItem = (item: NavItem) => {
    ...
    const linkContent = (
      <>
        ...
        <span className={cn(isMobile ? "" : "sr-only group-data-[state=expanded]:not-sr-only group-data-[state=expanded]:ml-2")}>
          {item.label}
        </span>
      </>
    );
      ...
      return (
        <RouterLink // Changed Link
          ...
          className={cn(
            ...
            isActive ? "text-sidebar-primary-foreground bg-sidebar-primary" : "text-sidebar-foreground hover:text-sidebar-primary-foreground hover:bg-sidebar-accent",
            "rounded-lg py-2"
          )}
        >
          {linkContent}
          ...
          <TooltipTrigger asChild>
            <RouterLink // Changed Link
              ...
              className={cn(
                ...
                isActive ? "bg-sidebar-primary text-sidebar-primary-foreground" : "text-sidebar-foreground hover:text-sidebar-primary-foreground hover:bg-sidebar-accent"
              )}
            >
              {linkContent}
            </RouterLink>
